const jsonData = JSON.parse(`[
  ...
  },
  {
    ...
    "dose": {
      ...
      "rules": [
        ...
        {
          "condition": "10 kg <= weight <= 20 kg",
          "d ose": "40 ml + 2 ml/kg * (weight - 10 kg) * 24",
          "unit": "ml"
        },
        {
          "condition": "weight > 20 kg",
  ...
  },
  {
    ...
    "indications": ["B acterial infections (e.g., sepsis, meningitis)", "Urinary tract infections"]
  },
  {
    "name": "GTN",
    "dose": 0.5,
...
const medicationsTable = document.getElementById('medications');
const medicationsBody = document.getElementById('medications-body');

// Function to filter medications by name
function filterMedicationsByName(name) {
  const filteredMedications = jsonData.filter(medication => medication.name.toLowerCase().includes(name.toLowerCase()));
  return filteredMedications;
}

// Function to filter medications by category
function filterMedicationsByCategory(category) {
  const filteredMedications = jsonData.filter(medication => medication.category === category);
  return filteredMedications;
}
...
searchInput.addEventListener('input', () => {
  ...
  const filteredMedications = filterMedicationsByName(searchValue);
  ...
  filteredMedications.forEach(medication => {
    ...
    doseCell.textContent = medication.dose;
    const frequencyCell = document.createElement('td');
    frequencyCell.textContent = medication.frequency;
    const formulaCell = document.createElement('td');
    formulaCell.textContent = medication.formula;
    const categoryCell = document.createElement('td');
    categoryCell.textContent = medication.category;
    const indicationsCell = document.createElement('td');
    indicationsCell.textContent = medication.indications.join(', ');
    row.appendChild(nameCell);
    row.appendChild(doseCell);
    row.appendChild(frequencyCell);
    row.appendChild(formulaCell);
    row.appendChild(categoryCell);
    row.appendChild(indicationsCell);
    medicationsBody.appendChild(row);
  });
});

// Event listener for calculate button
calculateButton.addEventListener('click', () => {
  const selectedMedication = medicationsTable.rows[1].cells[0].textContent;
  const selectedWeight = prompt('Enter the patient\'s weight in kg:');
  const medication = jsonData.find(medication => medication.name === selectedMedication);
  const dose = calculateDose(medication, selectedWeight);
  alert(`The calculated dose is: ${dose} ${medication.frequency}`);
});

// Initialize the medications table
jsonData.forEach(medication => {
  const row = document.createElement('tr');
  const nameCell = document.createElement('td');
  nameCell.textContent = medication.name;
  const doseCell = document.createElement('td');
